...
- ハッシュアルゴリズムはSHA-256を採用する **(仮決定)**。衝突耐性・速度ともに個人利用規模で十分であり、`sha2`クレートで標準的に扱えるため。
...
| `embed_color` | `NULL`ならデフォルト色として判定。フォーマットは16進カラーコード文字列(例: `0x87EB87`)に確定。フロントエンドではこの文字列を直接入力させず、0〜255のRGBスライダー等による視覚的な色選択UIを用意し、選択結果をこの文字列形式に変換して送信する。⚠️ デフォルト色そのものの具体的な値は未決定(TODO参照) |
...
### 12.4 設定ファイル・環境変数の命名規則(仮決定)
ブレスト段階では環境ラベルの区別方針のみが決まっていたため、YaakoDriveの前例に倣い階層的な上書き規則を追加する。
...
| 5 | embed_colorのデフォルト値 | `NULL`時に使用する具体的な16進カラーコード | 12.3 |
| 6 | 時間ベース/件数ベースクエリの具体値の妥当性検証 | 仮決定した「7日分」「50件」を、実運用データを見ながらPhase 12統合テスト時に調整 | 7.3 |
| 7 | 本番Cookie SecureフラグとTailscale運用の整合性 | Tailscale閉域内でSecure=trueを維持できるか(HTTP運用になる場合の扱い)の確認 | 11.2 |
| 8 | webhook_url暗号鍵のローテーション運用 | 鍵漏洩時の再暗号化手順など、運用フローは未設計 | 12.1, 18 |
| 9 | Phase 13〜14間のフロントエンド仮UIの要否 | 実スクレイピング実装(Phase13)完了後、本格フロントエンド実装(Phase14)までの間、動作確認用の簡易UIやAPI直叩き確認で足りるかの方針 | 2 |
