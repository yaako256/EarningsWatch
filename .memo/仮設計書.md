...
| # | 項目 | 内容 | 対応セクション |
|---|---|---|---|
| 1(解決済み) | Slack通知設定の詳細仕様 | `notify_slack_configs`のカラム構成、webhookの記法、mention_targetsの記法(`@here`/`@channel`等)。Slack Incoming Webhook / Block Kitの仕様確認が必要 | 8.7, 13.4 |
| 2(解決済み) | CSV/Excel業務ルールの異常値検知閾値 | 「明らかにおかしい値」判定の具体的な閾値。本設計時に決定する方針としてブレスト段階から据え置き | 9.3 |
| 3(解決済み) | mention_targetsの記法変換ロジック | Discord/Slackそれぞれのユーザ/ロール/全員メンションへの変換仕様。notify実装(Phase 13相当)時に決定 | 8.7, 12.3 |
| 4(解決済み) | スクレイピング判別用フィールド構成 | `EarningItems`の`fingerprint_item_*`が実際にどの情報(証券コード・公開日時・タイトル等)を持つかは対象サイトごとに実装時決定。Phase 13で確定 | 7.6 |
| 5(解決済み) | 時間ベース/件数ベースクエリの具体値の妥当性検証 | 仮決定した「7日分」「50件」を、実運用データを見ながらPhase 12統合テスト時に調整 | 7.3 |
| 6(解決済み) | 本番Cookie SecureフラグとTailscale運用の整合性 | Tailscale閉域内でSecure=trueを維持できるか(HTTP運用になる場合の扱い)の確認 | 11.2 |
| 7(解決済み) | webhook_url暗号鍵のローテーション運用 | 鍵漏洩時の再暗号化手順など、運用フローは未設計 | 12.1, 18 |
| 8(解決済み) | Phase 13〜14間のフロントエンド仮UIの要否 | 実スクレイピング実装(Phase13)完了後、本格フロントエンド実装(Phase14)までの間、動作確認用の簡易UIやAPI直叩き確認で足りるかの方針 | 2 |
| 9 | お知らせ板・パッチ通知の設計 | 新設セクション（15章 フロントエンド設計 配下、または新規章） |
| 10(解決済み) | フィルタデータのExcel出力（グループ別シート分け） | 14.11（フィルタAPI）/ 9章 |
| 11(解決済み) | フィルタの一括有効化/無効化/削除 | 14.11（フィルタAPI） |
